Rewrite the following LaTeX code in Russian.
Use ё. 
Please keep the LaTeX commands like \cdot, \bi, \norm, \kur, \tan, and so on as they are.
Please take into account the following:
image=образ
Proposition=Предложение
path connected=линейно связно

total curvature = полная кривизна
concatination = произведение
polygonal line = ломаная
rectifiable = спрямляемая
Frenet frame= базис Френе
Without loss of generality=Не умаляя общности
osculating=соприкасающийся
real=вещественный
moment curve=кривая моментов
signed curvature=ориентированная кривизна
supporting=опорная
supports=подпирает
cooriented=сонаправленный 
counteroriented=противонаправленный
Make a conclusion=Сделайте последний шаг
proper=собственный
circle=окружность
chart=карта
directional derivative=производная по направлению
unique=единственный
saddle surface=седловая поверхность
hat=горбушка
closest-point projection=короткая проекция
In fact=На самом деле
restriction=сужение
Parallel transport=Параллельный перенос
Gauss--Bonnet=Гаусса --- Бонне
Rauch=Раух
hinge=створка
...
сколь угодно --- произвольно
Для --- В
Без потери общности
Заметьте --- Убедитесь
можно выбрать --- найдётся
исполь... --- на
плоская --- на плоскости
может быть --- ся
в --- падеж
очевидно --- ясно
плоская кривая --- кривая на плоскости
пространственная кривая --- кривая в пространстве
обычно --- как правило
Будем --- Давайте
...
\begin{thm}{Продвинутое упражнение}\label{ex:pogorelov}
Пусть $f\:\Sigma\to \Sigma^*$ --- внутренняя изометрия между откытыми стого выпуклыми поверхностями.
Предположим, что ревенство $|p-x|=|f(p)-f(x)|$
выпоняется для некоторой фиксированной точки $p\in \Sigma$ и любой другой точки $x\in \Sigma$.
Докажите, что $f$ является сужением движения евклидова пространства на поверхность $\Sigma$.
\end{thm}
...
\parit{Источник:} \cite[III § 6]{pogorelov}.
