...
Если обратить ориентацию $\Sigma$, то главные кривизны сменят свои знаки и индексы:
$k_1$ превратится в $-k_2$, а $k_2$ в $-k_1$.
...
Это лучше подходит к названию, но оказывается неудобно.}
в точке~$p$.
...
Поэтому $\Shape_p$ действительно является линейным оператором $\T_p\Sigma\to \T_p\Sigma$ (это также будет следовать из \ref{thm:shape-chart}).

Оператор формы от касательного вектора $\vec w\z\in \T_p\Sigma$ может обозначаться как $\Shape\vec w$, если из контекста ясно, о какой базовой точке $p$ и о какой поверхности $\Sigma$ идет речь;
...
Пусть $(u,v)\mapsto s(u,v)$ --- гладкое отображение в гладкую поверхность $\Sigma$ с полем нормалей $\Norm$.
Тогда 
\begin{align*}
...
\end{align*}
где $\vec i,\vec j,\vec k$\index{10i@$\vec i$, $\vec j$, $\vec k$ (стандартный базис)} --- стандартный базис в $\mathbb{R}^3$.
...
Пусть $\Sigma$ --- гладкая поверхность гауссова кривизна которой не обращается в ноль.
Допустим, что её сферическое отображение $\Norm\:\Sigma\to\mathbb{S}^2$ инъективно.
Тогда 
...
=|K(p)|\ne 0\]
для любой точки $p\in\Sigma$.
В частности отображение $\Norm$ регулярно
и, поскольку оно инъективно, задаёт диффеоморфизм  $\Sigma\z\to\Norm(\Sigma)$.
...
Докажите, что $\Shape_p(\vec w)=\vec w$ при любых $p\in\Sigma$ и $\vec w\in \T_p\Sigma$.
\end{subthm}

\begin{subthm}{ex:normal-curvature=const:b}
Докажите, что точка $q\z= p+\Norm(p)$ не зависит от выбора $p\in\Sigma$.
...
Докажите, что $\Norm(u,v)$, $\vec u(u,v)$ и $\vec v(u,v)$ образуют ортонормированный базис при любых $(u,v)$.
\end{subthm}

\begin{subthm}{ex:flat-plane:depend}
Докажите, что $\Norm(u,v)$, $\vec u(u,v)$ и $\vec v(u,v)$ зависят только от $v$.
Выведите отсюда, что $u$-координатные линии являются отрезками прямых.
\end{subthm}
...
Упражнения \ref{ex:lin-ind-chart} и \ref{ex:line-cylinder} иллюстрируют другие части этого доказательства.
Упомянутая теорема была доказана Алексеем Погореловым \cite[II §3 теорема 2]{pogorelov1956} в гораздо более общих предположениях и позже была переоткрыта пару раз \cite{hartman-nirenberg,massey1962}.
...
$\tan(s),\mu(s),\Norm(s)$ образуют ориентированный ортонормированный базис.
Он называется \index{базис Дарбу}\emph{базис Дарбу} кривой $\gamma$ на~$\Sigma$.
...
Пусть $\Sigma$ --- поверхность вращения вокруг оси $x$ гладкой простой кривой с единичной скоростью $\gamma(s)=(x(s),y(s))$, лежащей в верхней полуплоскости.
Допустим, что $x'\ne 0$.
...
\frac{-y''(s)}{|x'(s)|}
\]
являются главными кривизнами $\Sigma$ в точке $(x(s),y(s),0)$
в направлениях соответствующих параллели и меридиана.
\end{subthm}
...
В случае $y=e^{-s}$ кривая \( \gamma \) называется \index{трактриса}\emph{трактрисой}, а поверхность — \index{псевдосфера}\emph{псевдосферой};
...
Начнём с трёхмерного варианта задачи о луне в луже (\ref{thm:moon-orginal}).
...
Позже (см. \ref{ex:convex-lagunov} и \ref{ex:rad=2})
...
(При их построении надо использовать процедуру сглаживания из приложения \ref{sec:analysis}.)
...
Упражнение \ref{ex:principal-revolution} приводит формулы для главных кривизн, применимые к граничным сферам $V_1$;
из них следует, что обе главные кривизны не превышают $1$ по абсолютной величине.

Остаётся сделать границу $V_1$ связной, не испортив при этом оценки на главные кривизны и не изменив максимальный радиус шаров внутри тела.
...
Теперь повторим операцию для других двух пар.
Пусть $V_3$ --- полученное тело.

Заметим, что граница $V_3$ связна.
...
это и прочее было доказано Владимиром Лагуновым и Абрамом Фетом \cite{lagunov-1960, lagunov-1961, lagunov-fet-1963, lagunov-fet-1965}.
...
При первом сверлении, мы получаем четыре дырки в поверхноти тела --- две сферы получают по одной дырке, и одна сфера две дырки.
...
Попробуйте изменить построение, чтобы граничная поверхность была сферой с тремя ручками.
\end{thm}
