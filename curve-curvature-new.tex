...
Кривизной полагается называть способ мерить, насколько геометрический объект отклоняется от \textit{прямого}, что бы последнее ни значило.
...
то есть, если $\vec v=\vec v(t)$ и $\vec w=\vec w(t)$ --- гладкие векторозначные функции вещественной переменной $t$, то
...
Покажите, что среди гладких простых пространственных кривых, только отрезки прямой имеют нулевую кривизну в каждой точке.
\end{thm}
...
Покажите, что кривизна любой гладкой сферической кривой не меньше $1$.
\end{thm}
...
то есть $\tan$ является сферической кривой.
...
Напомним, что гладкая кривая имеет гладкую регулярную параметризацию; см. \ref{sec:Smooth curves}.
Если $\gamma$ гладкая, то $t\mapsto \tan(t)$ задаёт гладкую параметризацию,
и она регулярна, если $|\tan'|=\kur\cdot|\gamma'|$ не обращается в нуль.
Отсюда следует, что \textit{если кривизна гладкой кривой не обнуляется, то её касательная индикатриса --- гладкая кривая}.
...
То есть, найдётся последовательность гладких кривых $\gamma_n\:\mathbb{I}\z\to\mathbb{R}^3$ с постоянной кривизной $1$ такая, что $\gamma_n(t)\to \gamma(t)$ для любого~$t$.
\end{thm}
...
$\gamma$.
Полная кривизна мерит насколько кривая поворачивает;
её также называют \index{вариация поворота}\emph{вариацией поворота}.
...
Другими словами, если $\gamma(s)=(x(s), y(s), z(s))$, то $z'(t)>0$ для любого~$t$.
Следовательно,
...
В частности, длина сферической кривой не превосходит её полной кривизны.
...
Напомним, что отображение монотонно, если прообраз любой точки в целевом пространстве связен (и в частности непуст).
...
Если $\gamma$ замкнута и $\gamma\:\mathbb{S}^1\to \mathbb{R}^2$ --- её параметризация, то её касательная индикатриса $\tan\:\mathbb{S}^1\to \mathbb{S}^1$ --- монотонное отображение.
\end{subthm}

\begin{subthm}{prop:convex-monotone:open}
Если $\gamma$ открыта и $\gamma\:\mathbb{R}\to \mathbb{R}^2$ --- её параметризация, то её касательная индикатриса $\tan\:\mathbb{R}\to\mathbb{S}^1$ определяет монотонное отображение на некоторый интервал в замкнутой полуокружности.
\end{subthm}
...
или же $y$-координата может иметь отрезок минимальных точек на $\gamma$, в этом случае, $\tan^{-1}\{\vec u\}$ --- дуга в~$\mathbb{S}^1$.
Следовательно, отображение $\tan\:\mathbb{S}^1\to \mathbb{S}^1$ монотонно.
...
Значит луч $h$ лежит в $F$, ведь $F$ замкнуто.
...
Тогда расстояние между конечными точками $\gamma_1$ не превосходит расстояние между конечными точками $\gamma_2$; то есть
...
\parbf{Доказательство леммы.}
Можно предположить, что $\gamma_1(a)\ne \gamma_1(b)$;
иначе нечего доказывать.
По выпуклости, кривая $\gamma_1$ лежит с одной стороны от прямой $\ell$, проходящей через $\gamma_1(a)$ и $\gamma_1(b)$;
будем думать, что $\ell$ направлена горизонтально и $\gamma_1$ лежит под ней.

Пусть $\gamma_1(s_0)$ будет самой низкой точкой на $\gamma_1$;
...
а $\tan_1$ и $\tan_2$ --- их касательные индикатрисы.
