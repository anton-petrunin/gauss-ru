...
\chapter[Подсказки]{\vspace*{-15mm} Подсказки\vspace*{-10mm}}
...
Образ $\gamma$ может выглядеть как цифра $8$ или $9$.
...
\parit{Источник:} Это рассуждение принадлежит Райнхарду Бёргеру \cite{borger}; см. также заметку Джереми Бразаса \cite{brazas}.
...
В \ref{SHORT.ex:cycloid:regular} нужно проверить, что $\gamma'_\ell\ne 0$.
В \ref{SHORT.ex:cycloid:simple} нужно проверить, что $\gamma_\ell(t_0)\z=\gamma_\ell(t_1)$ только если $t_0=t_1$.
...
Покажите, что прообраз $\gamma^{-1}(K)$ ограничен и замкнут в $\mathbb{R}$.
Далее примените лемму Гейне --- Бореля (\ref{thm:Heine--Borel}).
...
\[\tau_i=\max\set{\tau \in[0,1]}{\beta(\tau)=\gamma(t_i)}\]
при $i>0$.
...
Более того, равенство достигается тогда и только тогда, когда $\gamma$ простая.

\parbf{\ref{ex:integral-length}}; \ref{SHORT.ex:integral-length>}.
Примените формулу Ньютона --- Лейбница к каждому отрезку разбиения.
...
Покажите, что кривая с касательной индикатрисой $\tan$ решает задачу.
...
нам нужно построить ломаную $\beta$, вписанную в $\gamma$, для которой
...
Повторите доказательство \ref{thm:moon} для каждого циклического произведения дуги $\gamma$ от $p_i$ до $p_{i+3}$ с дугой окружности, на которой лежат точки $p_{i+4},\z\dots,p_{i-1}$.
...
Определите $h(x,y,z)\z\df \hat g(x,y)$, отсюда достаточность.
...
Покажите, что $s$ регулярно; то есть $s_u$ и $s_v$ линейно независимы при всех $u$ и $v$.

\parit{Замечание.}
Можно думать, что $s$ задаёт \index{стереографическая проекция}\emph{стереографическую проекцию}
...
В этом случае, $s\:(u,v)\z\mapsto (u,v,f(u,v))$ --- гладкая карта, накрывающая~$p$.
...
Воспользовавшись \ref{thm:ODE}, постройте кривую $\alpha$ на $\Sigma$, для которой $\alpha(0)=p$ и $\alpha'(x)\z=\vec x_{\alpha(x)}$.
...
Воспользовавшись срезками и сглаживаниями (\ref{sec:analysis}), постройте гладкую неотрицательную функцию $f$ на плоскости $(x,y)$, для которой $f(x,y)=0$ $\Leftrightarrow$ $(x,y)\in A$.
...
Воспользовавшись \ref{cor:Shape(ij)}, покажите, что $\Norm_v\z=-k \cdot s_v$.
...
\parit{Замечания.}
Это доказательство приводится в лекции Сергея Иванова \cite[3-й сем. Лекция 13]{ivanov}.
Другое доказательство получается применением формул Петерсона --- Кодацци.
...
Убедитесь, что плоскость, содержащая ось вращения, перпендикулярна поверхности, и примените \ref{thm:moon-orginal} к сечению тела этой плоскостью.

\parbf{\ref{ex:lagunov-genus4}.}
Просверлите ещё пару дырок или объедините два примера вместе.
Во второй части объедините $V_2$ and $V_3$.

\parit{Замечание.}
Можно построить такой пример с торической границей, но это очень продвинутое упражнение.
Тело, ограниченное сферой, обязано содержать шар радиуса чуть большего, чем $r_3=\sqrt{3/2}-1>r_2$; см. \cite{lagunov-1960, lagunov-1961, lagunov-fet-1963, lagunov-fet-1965}.
...
Можно считать, что $\Sigma_1$ задаётся неявно гладкой функцией $f$ в малой окрестности $U$ точки $p$.
Поскольку $\Sigma_1$ локально подпирает $\Sigma_2$ в точке $p$, можно считать, что $f(x)\ge 0$ для всех $x\in \Sigma_2\cap U$
и при этом $f(p)=0$.

Докажите, что $(f\circ\gamma_2)'(0)=0$ для любой гладкой кривой $\gamma_2\:(-\varepsilon,\varepsilon)\to\Sigma_2$ такой, что $\gamma_2(0)=p$.
Выведите отсюда, что вектор скорости $\gamma_2'(0)$ касателен к $\Sigma_1$ в $p$ и сделайте последний шаг.
...
Пусть $S$ --- проекция области $R$ на плоскость, проходящую через $p$, $q$ и центр шара.
...
По \ref{ex:rev(sin)} гауссова кривизна поверхности вращения графика $y=a\cdot \sin x$ для $x\in(0,\pi)$ не превосходит $1$.
...
То есть $\Sigma$ несобственная --- противоречие.
...
Пусть $a$ --- соответсвенная функция из утверждения выше.
...
разверните получившийся круговой сектор на плоскости.
...
Представим сначала, что ковбой перекинул через вершину верёвку и держится за оба её конца.
Тогда из картинки должно быть понятно, что у него получится подняться только если угол сектора строго меньше $\pi$.
Это дало бы ответ $\theta=\tfrac\pi3$.
Этот же ответ получится, если бы петлю в лассо заклинило, и оно не смогло бы затягиваться.
Ответ для затягивающегося лассо ощутимо больше, чем $\tfrac\pi3$.

\parit{Источник:}
Эту задачу мы узнали от Джоэля Файна, который приписывает её Фредерику Буржуа \cite{fine};
см. также \cite[Problem 2.10]{morin} и \cite[Problem 12]{khesin-tabachnikov}.
...
\parit{Замечание.}
Согласно \ref{ex:shape-curvature-line}, $\partial \Delta$ --- это линия кривизны в графике.
...
Первая кривая соответсвует геодезической на сглаженном конусе.
Убедитесь, что вторая и третья запрещаются рассуждением в доказательстве теоремы.
...
Допустим, что $b(r_1,\theta_1)<0$ при некоторой паре $(r_1,\theta_1)$ с $0<r_1<r_0$.
...
Применив \ref{prop:jaccobi}, выведите, что $b(r,\theta_1)=0$ для любого $r$.
...
\parit{Замечание.}
Последние два упражнения являются частными случаями теоремы Фердинанда Миндинга \cite{minding}: \textit{любые две поверхности с одной и той же постоянной гауссовой кривизной локально внутренне изометричны}.
...
\]
покажите, что $\cos\tilde\theta_n\to -1$, и, значит, $\tilde\theta_n\to \pi$ при $n\to\infty$.

\parbf{\ref{ex:thm:comp:cat:nsc}.}
Рассмотрите треугольник на гиперболоиде с вершинами $(1,0,0)$, $(-\tfrac12, \tfrac{\sqrt{3}}2,0)$ и $(-\tfrac12,-\tfrac{\sqrt{3}}2,0)$.
...
Рассмотрите последовательность точек определённую рекурентным соотношением $p_{n+1}\z\df q(p_n)$.
...
Докажите, что существует точка $p\in \Omega$ и последовательность $q_n\in Q$ такая, что $\dist{p}{q_n}{\spc{X}}<\tfrac1n$ для любого~$n$.
