...
Теоремы сравнения --- мощный инструмент, дающий возможность применять евклидову интуицию в дифференциальной геометрии.
...
а кратчайшие $[x,y]_\Sigma$, $[y,z]_\Sigma$ и $[z,x]_\Sigma$ --- его {}\emph{сторонами};
...
и называется \index{модельный треугольник и угол}\emph{модельным углом} треугольника $[xyz]$ при~$x$.
...
В частности, \ref{thm:comp:cat} неверно без предположения об односвязности.
\end{thm}
...
в частности, у обеих сторон равенства тот же знак:
\begin{itemize}
...
то есть $\dist{p}{q}\Sigma\z\ge\dist{x}{y}\Sigma$ при любых $x,y\in \Sigma$.
...
<
\frac{2}{3}\cdot\ell$.
Тогда сравнение \ref{eq:key-lem:globalization} выполняется и в случае, если $\dist{x}{y}\Sigma+\dist{x}{z}\Sigma<\ell$.
\end{thm}
...
Следующее вычислительное упражнение будет использовано в доказательстве леммы.
...
=\tfrac{2}{3}\cdot\ell
\eqlbl{3|xx'|}\]
Далее выберем геодезическую $[x', p]$ и рассмотрим створку $\hinge{x'}p q$, образованную $[x',p]$ и $[x',q]\z\subset [x,q]$.
...
Последовательность может остановиться на $x_n$ только если $r_n\z< \tfrac{2}{3}\cdot\ell$.
...
Осталось доказать \ref{eq:thm:=def-loc} в случае если последовательность $x_n$ бесконечна.
Из \ref{3|xx'|}
\[
...
\begin{subthm}{}
Покажите, что если $\Sigma$ односвязна и её гауссова кривизна неположительна, то функция $h$ является выпуклой.
\end{subthm}

\begin{subthm}{}
Покажите, что если гауссова кривизна $\Sigma$ неотрицательна, то функция $h$ является вогнутой.
\end{subthm}
...
Докажите, что площадь любого $R$-шара во внутренней метрике $\Sigma$ не превышает $\pi\cdot R^2$.
\end{thm}
...
Пусть $\Delta$ --- топологический диск на гладкой поверхности $\Sigma$ неположительной гауссовой кривизны.
